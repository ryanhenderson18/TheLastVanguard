...
public class SpawnerL4 : MonoBehaviour
{
    ...
    IEnumerator spawnEnemies()
    {
        while (clones.Count < maxSpawned)
        {
            ...
            if (number < 3)
            {
                ...
                else if (randomEnemyChoice >= 50 && randomEnemyChoice < 75)
                {
                    clones.Add(Instantiate(mediumEnemy, spawnPoints[spawnChoice], Quaternion.identity));
                }
                else if (randomEnemyChoice >= 75 && getActiveHardEnemies() == 0)
                {
